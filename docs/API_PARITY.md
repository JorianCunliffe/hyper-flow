...
| Files, recordings and attachments | HyperFlow `/api/files`, `/?files=1`, stable authenticated file pages | Managed upload, resumable chunks, private downloads, metadata pages and deletion implemented and locally tested. Existing upload controls and public Ask uploads use this path. Activation remains disabled pending storage provisioning and billing approval. See [file runbook](MANAGED_FILES.md). |
| Erasure, retention and restoration | Separate app lifecycle procedures | Communications local database erasure and HyperFlow `erase_database` are separate released operations. Managed-object erasure has its own human-admin command, exact confirmation, progress and retained-file receipts. Storage activation is pending. External provider resources, backups and automatic retention remain separate work. Portable exports are not a tested full backup/restore service. |
