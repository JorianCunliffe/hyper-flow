...
| Erasure, retention and restoration | Separate app lifecycle procedures | Communications local database erasure exists. HyperFlow `erase_database` removes its business database roots after suspension, export review and an independently verified Communications closed receipt; it retains file manifests, identities, membership and recovery audit. Files, external provider resources, backups and automatic retention remain separate work. Portable exports are not a tested full backup/restore service. |
